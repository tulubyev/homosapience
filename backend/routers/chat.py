...
# ── Admin + Gold caches (DB-backed, refreshed every 60s) ────────────────────
...
_admin_cache: set[str] = set(_env_admin_dids)  # role == 'admin' only
_gold_cache:  set[str] = set()                  # role == 'gold_member' only
...
async def _refresh_admin_cache(db) -> None:
    """Reload admin/gold DID shorts from DB (TTL-cached)."""
    global _admin_cache, _gold_cache, _admin_cache_ts
    now = time.time()
    if now - _admin_cache_ts < _ADMIN_CACHE_TTL:
        return
    try:
        rows = await db.get_admin_dids(active_only=True)
        _admin_cache = {r["did_short"] for r in rows if r.get("role") == "admin"}
        _gold_cache  = {r["did_short"] for r in rows if r.get("role") == "gold_member"}
        _admin_cache_ts = now
    except Exception:
...
def _is_gold(sender_short: str) -> bool:
    return bool(sender_short and sender_short in _gold_cache)
...
def _row_to_msg(row: dict) -> dict:
    ...
    # Enrich with live role caches.
    # Gold-only members were historically stored with is_creator=True (bug).
    # Override: is_creator must be False for gold-only senders.
    short = msg.get("sender_short", "")
    gold  = _is_gold(short)
    admin = _is_admin(short)
    msg["is_gold"] = gold
    if gold and not admin:
        msg["is_creator"] = False
...
class UnreadReq(BaseModel):
    since:         dict[str, int] = {}    # roomId → caller's last-read ts
    default_since: int = 0                # for rooms not in `since`
    me:            Optional[str] = None   # exclude the caller's own messages


@router.post("/unread")
async def unread_counts(body: UnreadReq, request: Request):
    """
    Per-room unread counts for the channel badges + header chat icon. Unread =
    non-system, non-DM messages in a room newer than the caller's last-read for
    that room (or `default_since`), excluding the caller's own messages. Public —
    rooms are shared; the last-read state lives client-side.
    """
    pool = _get_pool(request)
    if pool is None:
        return {"counts": {}, "total": 0}
    cutoff = int(time.time()) - 30 * 86400  # bound the scan to recent history
    async with pool.acquire() as conn:
        rows = await conn.fetch(
            "SELECT room, timestamp, sender_short FROM chat_messages "
            "WHERE is_system = FALSE AND timestamp >= $1",
            cutoff,
        )
    counts: dict[str, int] = {}
    for r in rows:
        ...
        if room_id.startswith("dm_"):
            # Include DMs only when we know who the caller is and they're a participant
            if not body.me:
                continue
            parts = room_id[3:].split("_", 1)  # "abcd1234_xyz789" → ["abcd1234", "xyz789"]
            if body.me not in parts:
                continue
        if body.me and r["sender_short"] == body.me:
            continue
        since = body.since.get(room_id, body.default_since)
