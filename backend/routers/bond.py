...
async def create_bond_request(body: BondRequestCreate, request: Request):
    ...
    # ── Shielded Human: shielded credentials stay at newcomer (phase 1) ───────
    # They never accumulate bonds, so trust cannot be elevated. Blocking the
    # request here is the single enforcement point (the requester is what gets
    # upgraded on threshold). Unlinkable reputation transfer is a future ZK cycle.
    _req_cred = await db.get_credential(body.requester_did) if body.requester_did else None
    if _req_cred and _req_cred.get("mode") == "shielded":
        raise HTTPException(
            status_code=403,
            detail={
                "error": "shielded_no_bonds",
                "message": "Shielded credentials stay at newcomer trust and cannot request bonds.",
            },
        )
