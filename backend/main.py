...
async def lifespan(app: FastAPI):
    ...
    # R1: embed assertion signing key (fail-safe — required when EMBED_API on)
    if feature_enabled("EMBED_API"):
        from services.server_key import get_server_key
        _sk = get_server_key()
        if not _sk.available:
            raise RuntimeError(
                "EMBED_API is enabled but APTOGON_JWT_PRIVATE_KEY is not set/invalid. "
        ...
        print("  ✅ EMBED_API: server signing key loaded")
