...
# A hard block additionally REQUIRES one of these strong automation artifacts —
# a stack of soft fingerprint heuristics alone must never lock a real user out
# (mobile browsers legitimately trip several soft signals). `challenge_too_fast`
# is a human-impossible reaction time, so it counts as a strong bot indicator too.
_HARD_BLOCK_SIGNALS = {"webdriver", "cdp_artifact", "headless_ua", "challenge_too_fast"}
...
class RiskEngine:
    ...
    def assess(
        ...
        # ── Hard-block gate ───────────────────────────────────────────────────
        # A HARD block (user cannot even attempt the gesture) requires at least one
        # STRONG automation artifact. The weighted sum alone must never hard-block:
        # real phones legitimately stack soft signals (no_plugins, perm_denied,
        # audio_anomaly on suspended mobile AudioContext, no_scroll_events, …) that
        # can cross 0.85 with zero real automation. Soft-only high risk → step_up
        # (a harder challenge), never an outright block. The gesture AI is the real
        # test; we don't lock a human out on fingerprint heuristics alone.
        strong_automation = bool(fired_set & _HARD_BLOCK_SIGNALS)
        blocked = (score >= _THRESH_BLOCK) and strong_automation
        ...
        return RiskResult(
            ...
            blocked=blocked,
            step_up=(score >= _THRESH_HUMAN and not blocked),
