...
# ── Feature flags ─────────────────────────────────────────────────────────────
# FEATURE_VERIFY_COOLDOWN=false disables the 30-min IP cooldown after failures.
# Production: true (default). Dev/red-team testing: false.
FEATURE_VERIFY_COOLDOWN = os.getenv("FEATURE_VERIFY_COOLDOWN", "true").lower() != "false"
...
SYBIL_FP_LIMIT          = 5     # unique fp_hashes per IP per hour before sybil flag
...
class RateLimiter:
    ...
    async def check_verify_ip(self, ip: str) -> tuple[bool, str]:
        ...
        """
        # 1. Cooldown check (too many failures) — skipped when FEATURE_VERIFY_COOLDOWN=false
        cooldown_key = f"verify:cooldown:{ip}"
        if FEATURE_VERIFY_COOLDOWN and await self._exists(cooldown_key):
    ...
    async def record_verify_failure(self, ip: str):
        ...
        if not FEATURE_VERIFY_COOLDOWN:
            return
    ...
    async def check_verify_sybil(self, ip: str, fp_hash: str) -> tuple[bool, str]:
        """
        Sybil detection: track unique fp_hashes per IP within the current hour.
        If the same IP registers more than SYBIL_FP_LIMIT distinct fingerprints,
        it's flagged as a fingerprint-rotation sybil attack (A3 red-team vector).
        Returns (allowed: bool, reason: str).
        """
        if not fp_hash:
            return True, "ok"
        hour_slot = int(time.time() // 3600)
        key = f"sybil:fp:{ip}:{hour_slot}"
        if self._redis:
            try:
                await self._redis.sadd(key, fp_hash)
                count = await self._redis.scard(key)
                if count == 1:
                    await self._redis.expire(key, 3600)
                if count > SYBIL_FP_LIMIT:
                    log.warning("Sybil detected: IP=%s unique_fp=%d", ip, count)
                    return False, (
                        f"Too many distinct device fingerprints from this IP "
                        f"({count}/{SYBIL_FP_LIMIT} per hour). Possible sybil attack."
                    )
                return True, "ok"
            except Exception as e:
                log.warning("Redis error in check_verify_sybil: %s", e)
                return True, "ok"
        # In-memory fallback: use a set stored in the mem dict
        now = time.time()
        entry = self._mem.get(key)
        if entry is None or now > entry["expires"]:
            self._mem[key] = {"val": {fp_hash}, "expires": now + 3600}
            return True, "ok"
        entry["val"].add(fp_hash)
        count = len(entry["val"])
        if count > SYBIL_FP_LIMIT:
            log.warning("Sybil detected (in-memory): IP=%s unique_fp=%d", ip, count)
            return False, (
                f"Too many distinct device fingerprints from this IP "
                f"({count}/{SYBIL_FP_LIMIT} per hour)."
            )
        return True, "ok"
