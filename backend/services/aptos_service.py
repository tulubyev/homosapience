...
class AptosService:
    ...
    def __init__(self):
        ...
        # Serialises all on-chain transactions from this process so concurrent
        # verifications never read the same sequence_number and cause one tx to fail.
    ...
    async def _submit_tx(self, function: str, args: list) -> str:
        ...
        Все блокирующие urlopen-вызовы выполняются в ThreadPoolExecutor чтобы не
        блокировать asyncio event loop. Lock гарантирует что только одна транзакция
        в процессе одновременно — это устраняет race по sequence_number.
        """
        import asyncio

        # Lazy-init Lock (нельзя создать в __init__ до запуска event loop в < Python 3.10)
        if self._tx_lock is None:
            self._tx_lock = asyncio.Lock()

        async with self._tx_lock:
            return await self._submit_tx_locked(function, args)

    async def _submit_tx_locked(self, function: str, args: list) -> str:
        """Inner implementation — called only while _tx_lock is held."""
        import asyncio
        ...
        def _http_post(url: str, payload: bytes, timeout: int = 15) -> dict:
            req = Request(url, data=payload,
                          headers={"Content-Type": "application/json"}, method="POST")
            with urlopen(req, timeout=timeout) as resp:
                return json.loads(resp.read())
        ...
        # 1. Получаем sequence_number (non-blocking)
        acct_url = f"{self.node_url}/accounts/{signer}"
        acct_data = await loop.run_in_executor(None, _http_get, acct_url)
        ...
        # 3. Encode submission (non-blocking)
        encode_url = f"{self.node_url}/transactions/encode_submission"
        signing_message_hex: str = await loop.run_in_executor(
            None, _http_post, encode_url, json.dumps(unsigned_tx).encode()
        )
        ...
        # 5. Отправляем подписанную транзакцию (non-blocking)
        signed_tx = {
            **unsigned_tx,
            "signature": {
        ...
        result = await loop.run_in_executor(
            None, _http_post, submit_url, json.dumps(signed_tx).encode()
        )

        tx_hash = result["hash"]
        print(f"✅ Aptos TX submitted: {tx_hash}")

        # 6. Ждём подтверждения (polling — urlopen в executor, sleep yields event loop)
        confirmed = False
        for _ in range(20):
            ...
            try:
                ...
                tx_data = await loop.run_in_executor(
                    None, lambda: _http_get(check_url, timeout=5)
                )
