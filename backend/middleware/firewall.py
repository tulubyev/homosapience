...
PUBLIC_PREFIXES = (
    ...
    "/api/research/study/gesture",  # ← lab study: volunteers have no DID; gated by study code

    "/api/founders",       # ← public roster — founders are public by design
    "/api/embed",          # ← R1: challenge/assert/jwks public; verify auth'd by sk
    "/api/captcha",        # ← gesture-CAPTCHA: verify auth'd by pk↔allowed_origins,
                           #   siteverify by sk_live_ bearer (not a DID session JWT)
    "/api/agent/verify",   # ← HDAA: public token verification (no auth — like /api/risk/stats)
    "/api/console/account/verify",  # ← self-serve email magic-link, opened from the
                                    #   email with no DID; the token itself is the proof
    "/uploads",
    "/ws",
)
