...
const STEP_ICONS = ['📋', '✍️', '🔑', '🧮']
const STEP_COLORS = ['#db2777', '#7c3aed', '#059669', '#2563eb']
const FEATURE_ICONS = ['🎯', '🔬', '💰']
...
export default function AILabelingPage() {
  const t = useTranslations('solutions_ai_labeling')
  ...
  const features = t.raw('quality.features') as FeatureItem[]
  ...
  return (
    <div style={{ background: '#f8fafc', minHeight: '100vh', fontFamily: 'Inter, system-ui, sans-serif' }}>
      ...
      {/* ── HERO ── */}
      <section style={{ background: 'linear-gradient(135deg, #1a0a2e 0%, #2d1b4e 100%)', padding: '72px 24px 56px', textAlign: 'center' }}>
        <div style={{ maxWidth: 700, margin: '0 auto' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(219,39,119,0.2)', border: '1px solid rgba(249,168,212,0.3)', borderRadius: 99, padding: '6px 16px', marginBottom: 28 }}>
            <span style={{ fontSize: 14 }}>🤖</span>
            <span style={{ fontSize: 11, fontWeight: 700, color: '#f9a8d4', letterSpacing: '0.12em', textTransform: 'uppercase' }}>{t('hero.eyebrow')}</span>
          </div>
          ...
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
            <Link href="/verify" style={{ padding: '13px 32px', background: 'linear-gradient(135deg,#db2777,#7c3aed)', color: '#fff', fontWeight: 700, fontSize: '0.95rem', borderRadius: 12, textDecoration: 'none', boxShadow: '0 6px 24px rgba(219,39,119,0.35)' }}>
              {t('hero.cta_try')} →
            </Link>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(190px, 1fr))', gap: 14 }}>
            {problems.map(p => (
              <div key={p.stat} style={{ background: '#f8fafc', borderRadius: 14, border: '1px solid #e5e7eb', padding: '20px 16px' }}>
                <div style={{ fontSize: '1.6rem', fontWeight: 900, color: '#db2777', marginBottom: 6 }}>{p.stat}</div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {vs.map(r => (
              <div key={r.name} style={{
                background: r.verdict === 'good' ? '#fdf2f8' : '#f8fafc',
                border: `1px solid ${r.verdict === 'good' ? '#f9a8d4' : '#e5e7eb'}`,
                borderInlineStart: `3px solid ${r.verdict === 'good' ? '#db2777' : '#e5e7eb'}`,
                borderRadius: 10, padding: '14px 18px',
                display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 16,
              }}>
                <span style={{ fontWeight: 800, fontSize: '0.9rem', color: r.verdict === 'good' ? '#db2777' : '#111827', minWidth: 160 }}>{r.name}</span>
                <span style={{ fontSize: '0.82rem', color: r.verdict === 'good' ? '#9d174d' : '#6b7280', flex: 1 }}>{r.res}</span>
      ...
      {/* ── QUALITY GUARANTEE ── */}
      <section style={{ padding: '60px 24px', background: '#1a0a2e' }}>
        <div style={{ maxWidth: 700, margin: '0 auto', textAlign: 'center' }}>
          <h2 style={{ fontWeight: 900, fontSize: '1.5rem', color: '#f1f5f9', marginBottom: 16 }}>{t('quality.title')}</h2>
      ...
      {/* ── CTA ── */}
      <section style={{ padding: '64px 24px', background: 'linear-gradient(135deg, #fdf2f8 0%, #f5f3ff 100%)', textAlign: 'center' }}>
        <div style={{ maxWidth: 560, margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
            <Link href="/verify" style={{ padding: '14px 36px', background: 'linear-gradient(135deg,#db2777,#7c3aed)', color: '#fff', fontWeight: 700, fontSize: '1rem', borderRadius: 14, textDecoration: 'none', boxShadow: '0 6px 24px rgba(219,39,119,0.3)' }}>
              {t('cta.start_free')} →
            </Link>
      ...
      <div style={{ background: '#fff', borderTop: '1px solid #f1f5f9', padding: '16px 24px' }}>
        <div style={{ maxWidth: 900, margin: '0 auto', display: 'flex', gap: 16, flexWrap: 'wrap' }}>
          ...
          {NAV_LINKS.map((l, i) => (
            <Link key={l.href} href={l.href} style={{ fontSize: '0.8rem', color: '#db2777', textDecoration: 'none', fontWeight: 600 }}>{l.icon} {navLabels[i].label}</Link>
          ))}
        </div>
      </div>
    </div>
