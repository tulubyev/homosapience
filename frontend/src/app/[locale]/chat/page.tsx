...
interface Message {
  ...
  is_system?: boolean; is_creator?: boolean
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        ...
        <aside className="chat-sidebar-left" style={{ background: '#f3f0ff', borderRight: '1px solid #e9d5ff', display: 'flex', flexDirection: 'column', flexShrink: 0, overflowY: 'auto' }}>
          ...
          {allRooms.map(room => {
            ...
            const unread = !dmPartner && room.id !== currentRoom && messages.some(m => m.room === room.id && !m.is_system)
            ...
            return (
              <div key={room.id} style={{ position: 'relative', display: 'flex', alignItems: 'center' }}
                ...
                <button
                  ...
                  style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 10, padding: '9px 12px', background: active ? 'rgba(124,58,237,0.14)' : 'none', border: 'none', cursor: 'pointer', textAlign: 'left', borderLeft: active ? '3px solid #7c3aed' : '3px solid transparent', transition: 'background 0.15s' }}
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 12, fontWeight: active ? 700 : 500, color: active ? '#4c1d95' : '#374151', display: 'flex', alignItems: 'center', gap: 5 }}>
                      #{room.label}
                      {unread && <span style={{ width: 5, height: 5, borderRadius: '50%', background: '#7c3aed', display: 'inline-block' }} />}
          ...
          {isVerified && (
            <button onClick={() => setCreateRoomOpen(true)}
              style={{ margin: '4px 8px', padding: '7px 10px', background: 'none', border: '1px dashed #c4b5fd', borderRadius: 8, cursor: 'pointer', fontSize: 11, color: '#7c3aed', fontWeight: 600, textAlign: 'left' }}>
              ＋ Create room
            </button>
          )}
          ...
          {dmSearchOpen && (
            <div style={{ padding: '4px 8px 8px' }}>
              ...
              />
              <div style={{ fontSize: 10, color: '#94a3b8', marginTop: 3, paddingLeft: 2 }}>Press Enter to open DM</div>
            </div>
          )}
          ...
          {dmList.map(dm => {
            ...
            return (
              <button
                ...
                style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 12px', background: activeDM ? 'rgba(124,58,237,0.14)' : 'none', border: 'none', cursor: 'pointer', textAlign: 'left', borderLeft: activeDM ? '3px solid #7c3aed' : '3px solid transparent', width: '100%' }}
              >
                  ...
                  <div style={{ maxWidth: '62%' }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3, paddingLeft: 4 }}>
                        {msg.is_creator ? (
                          <>
                            ...
                            <span style={{ fontSize: 10, background: 'linear-gradient(135deg,#7c3aed,#0891b2)', color: '#fff', padding: '1px 6px', borderRadius: 99, fontWeight: 700 }}>admin</span>
                          </>
                        ) : (
                          <>
                            <span
                    ...
                    {msg.reply_to && (
                      <div style={{ background: 'rgba(124,58,237,0.08)', borderLeft: '3px solid #7c3aed', padding: '4px 10px', borderRadius: '8px 8px 0 0', fontSize: 11, color: '#6b7280', marginBottom: -6, paddingBottom: 10 }}>
                        ...
                        {': '}{(() => {
                          const fm = parseFileMeta(msg.reply_to.content)
                          if (fm) return `📄 ${fm.name}`
                          return msg.reply_to.content.length > 60 ? msg.reply_to.content.slice(0, 60) + '…' : msg.reply_to.content
                        })()}
                      </div>
                    )}
                    ...
                    <div style={{ background: isMine ? 'linear-gradient(135deg, #7c3aed, #2563eb)' : '#fff', color: isMine ? '#fff' : '#111827', padding: parseFileMeta(msg.content) ? '8px 10px' : '10px 14px', borderRadius: isMine ? (msg.reply_to ? '4px 18px 4px 18px' : '18px 18px 4px 18px') : (msg.reply_to ? '18px 4px 18px 4px' : '18px 18px 18px 4px'), fontSize: 14, lineHeight: 1.55, boxShadow: '0 1px 4px rgba(0,0,0,0.08)', wordBreak: 'break-word', display: 'flex', alignItems: 'flex-end', gap: 8 }}>
                      {parseFileMeta(msg.content)
                        ...
                        : <><span style={{ flex: 1 }}>{renderContent(msg.content, openDM, myShort)}</span>
                          {isMine && <span style={{ fontSize: 10, opacity: 0.65, flexShrink: 0 }}>{fmtTime(msg.timestamp)}</span>}</>
                      }
                    </div>
                    ...
                    {translations[msg.id] && (
                      <div style={{ fontSize: 12, color: '#6b7280', fontStyle: 'italic', marginTop: 4, textAlign: isMine ? 'right' : 'left', paddingLeft: isMine ? 0 : 4 }}>
                        🌐 {translations[msg.id]}
                      </div>
                    )}
                  </div>
          ...
          {replyTo && !dmPartner && (
            <div style={{ background: '#f3f0ff', borderTop: '2px solid #ddd6fe', padding: '8px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
              <div style={{ fontSize: 12, color: '#4b5563', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                ↩️ <span style={{ fontWeight: 700, color: '#7c3aed', fontFamily: 'monospace' }}>{replyTo.sender_short}</span>{': '}{(() => { const fm = parseFileMeta(replyTo.content); return fm ? `📄 ${fm.name}` : (replyTo.content.length > 80 ? replyTo.content.slice(0, 80) + '…' : replyTo.content) })()}
              </div>
              <button onClick={() => setReplyTo(null)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#9ca3af', fontSize: 16, marginLeft: 12, flexShrink: 0 }}>✕</button>
            </div>
          )}
