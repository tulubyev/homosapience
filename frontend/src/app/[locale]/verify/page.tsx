...
import { getSessionToken, revokeAllSessions, getActiveSessions } from '@/lib/sessionAuth'
...
export default function VerifyPage() {
  ...
  const handleGesture = async (events: TouchEventData[], challenges: ChallengeResult[]) => {
    ...
    try {
      // R2: submit risk signals in parallel with fingerprint collection (fail-open)
      ...
      if (data.passed && data.did) {
        ...
        if (fpHash) localStorage.setItem('hsi_fp_hash', fpHash)
        ...
        if (data.private_key_b64) {
          getSessionToken(data.did, data.private_key_b64).catch(() => {/* non-critical */})
        }
  ...
  return (
    <div style={{ background: '#f8fafc', minHeight: '100vh', fontFamily: 'Inter, system-ui, sans-serif' }}>
      ...
      <div style={{ maxWidth: 640, margin: '0 auto', padding: '40px 24px 60px' }}>
        ...
        {!linkCode && !consented && !importSuccess && (
          <div style={{ background: '#f8fafc', border: '1.5px solid #e2e8f0', borderRadius: 16, padding: '14px 20px', marginBottom: 20 }}>
        ...
        {!consented && !importSuccess && (
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 20 }}>
              ...
              <div>
                ...
                <p style={{ margin: 0, fontSize: 13, color: '#94a3b8', marginTop: 2 }}>
                  Please read how your data is used
                </p>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 24 }}>
              {[
                { icon: '✏️', title: 'Gesture recording', desc: 'Your drawing motion (touch/mouse events) will be recorded for ~10 seconds and sent to our AI for human-pattern analysis.' },
                { icon: '🤖', title: 'AI analysis', desc: 'The gesture is analyzed by SapiX (via OpenRouter / Qwen3-14B). No image or video is captured — only movement vectors.' },
                { icon: '🔑', title: 'Anonymous DID', desc: 'A cryptographic identifier (DID) is created locally in your browser. It contains no personal information — no name, email, or IP.' },
                { icon: '🗑️', title: 'Data retention', desc: 'Raw gesture data is not stored. Only an anonymized SHA-3 hash of the motion proof is recorded on Aptos blockchain.' },
                { icon: '📍', title: 'Device fingerprint', desc: 'A one-way hash of your device characteristics (screen, language, timezone) is used to prevent duplicate registrations. Never transmitted in raw form.' },
              ].map(item => (
            ...
            <p style={{ fontSize: 11, color: '#94a3b8', marginBottom: 20, lineHeight: 1.6 }}>
              By continuing you consent to gesture analysis under{' '}
              <a href="/privacy" style={{ color: '#7c3aed' }}>APTOGON Privacy Policy</a>.
              {' '}This processing is based on your explicit consent (GDPR Art. 6(1)(a)).
              You may withdraw at any time by deleting your credential from browser storage.
            </p>
        ...
        {consented && (stage === 'draw' || stage === 'failed') && (<>
          ...
          <div style={{ background: '#fff', borderRadius: 24, padding: 28, border: '2px dashed rgba(124,58,237,0.3)', boxShadow: '0 4px 32px rgba(124,58,237,0.08)' }}>
            ...
            <div style={{ marginTop: 10, padding: '8px 14px', background: '#f0f9ff', borderRadius: 10, border: '1px solid #bae6fd', fontSize: 12, color: '#0369a1', lineHeight: 1.5 }}>
              💡 Во время рисования может появиться цветная точка — нажми на неё не отрывая руку от экрана
            </div>
              ...
              <div style={{ background: 'linear-gradient(135deg,#7c3aed,#2563eb)', borderRadius: 20, padding: '18px 22px', color: '#fff', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 14, boxShadow: '0 4px 24px rgba(124,58,237,0.25)' }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 13, opacity: 0.9 }}>
                    ...
                    {pairResult.is_admin && <span style={{ marginLeft: 6, background: 'rgba(255,255,255,0.2)', padding: '1px 8px', borderRadius: 99, fontSize: 11, fontWeight: 700 }}>👤 {pairResult.role}</span>}
                  </div>
                </div>
              </div>
            )}
