    ...
    <div style={{ background: '#f8fafc', minHeight: '100vh', fontFamily: 'Inter, system-ui, sans-serif' }}>
        ...
        <div style={{ position: 'relative', zIndex: 1, maxWidth: 700, margin: '0 auto' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 16, marginBottom: 24, flexWrap: 'wrap' }}>
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              <HumanBadge state="verified"   size={32} title={t('badge_states.verified')} />
              <HumanBadge state="unverified" size={32} title={t('badge_states.unverified')} />
              <HumanBadge state="pending"    size={32} title={t('badge_states.pending')} />
              <HumanBadge state="revoked"    size={32} title={t('badge_states.revoked')} />
            </div>
          ...
          <p style={{ fontSize: 'clamp(1.15rem,3vw,1.55rem)', fontWeight: 800, color: '#111827', marginBottom: 8, lineHeight: 1.25 }}>
            {t('hero_tagline_1')}<br />{t('hero_tagline_2')}
          </p>
          ...
          {/* Stat strip */}
          <div style={{ display: 'flex', gap: 32, justifyContent: 'center', flexWrap: 'wrap', marginTop: 36, paddingTop: 28, borderTop: '1px solid rgba(124,58,237,0.1)' }}>
            {/* The "~10 sec" figure is a design target, not a measurement — the
                source label used to say "Our data", which nothing in the repo
                backs up. Now sourced honestly, and localised like the rest. */}
            {(t.raw('stats') as { n: string; l: string; src: string }[]).map(({ n, l, src }) => (
              <div key={n} style={{ textAlign: 'center' }}>
                <div style={{ fontSize: '1.5rem', fontWeight: 900, color: '#7c3aed', lineHeight: 1 }}>{n}</div>
                <div style={{ fontSize: '0.75rem', color: '#374151', fontWeight: 600, marginTop: 4 }}>{l}</div>
                <div style={{ fontSize: '0.65rem', color: '#9ca3af', marginTop: 2 }}>{src}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section style={{ padding: '64px 24px', background: '#f8fafc' }}>
        <div style={{ maxWidth: 900, margin: '0 auto' }}>
          <p style={{ textAlign: 'center', fontWeight: 900, fontSize: '1.5rem', color: '#111827', marginBottom: 8 }}>{t('verticals.title')}</p>
          <p style={{ textAlign: 'center', color: '#6b7280', marginBottom: 36, fontSize: '1rem' }}>
            {t('verticals.subtitle')}
          </p>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 16 }}>
            {/* Presentation stays in code, wording comes from the catalogue —
                paired by index so a translator never touches colours or routes. */}
            {([
              ...
              const copy = (t.raw('verticals.items') as { title: string; desc: string }[])[i]
              const v = { ...style, ...copy, cta: t('verticals.cta') }
              return (
              <Link key={v.href} href={v.href} style={{ textDecoration: 'none', display: 'flex', flexDirection: 'column', background: '#fff', border: `1px solid ${v.border}`, borderTop: `3px solid ${v.color}`, borderRadius: 16, padding: '22px 18px', transition: 'box-shadow 0.15s' }}>
                <div style={{ width: 44, height: 44, borderRadius: 12, background: v.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22, marginBottom: 12 }}>{v.icon}</div>
                <div style={{ fontWeight: 800, fontSize: '1rem', color: '#111827', marginBottom: 6 }}>{v.title}</div>
                <p style={{ fontSize: '0.82rem', color: '#6b7280', lineHeight: 1.55, margin: '0 0 14px', flex: 1 }}>{v.desc}</p>
                <span style={{ fontSize: '0.8rem', fontWeight: 700, color: v.color }}>{v.cta} →</span>
              </Link>
              )
            })}
          </div>
      ...
      <section style={{ padding: '48px 24px', background: '#fff' }}>
        <div style={{ maxWidth: 900, margin: '0 auto' }}>
          <p style={{ textAlign: 'center', fontWeight: 900, fontSize: '1.5rem', color: '#111827', marginBottom: 8 }}>{t('why.title')}</p>
          <p style={{ textAlign: 'center', color: '#6b7280', marginBottom: 28, fontSize: '0.95rem' }}>
            {t('why.subtitle')}
          </p>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              <thead>
                <tr style={{ background: '#f8fafc', borderBottom: '1px solid #e5e7eb' }}>
                  <th style={{ padding: '10px 14px', textAlign: 'start', fontSize: 11, fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase' as const }}></th>
                  {(['VerifyYou', 'Worldcoin', 'KYC'] as const).map(c => (
                    <th key={c} style={{ padding: '10px 14px', textAlign: 'center', fontSize: 11, fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase' as const }}>{c}</th>
                  ))}
                  <th style={{ padding: '10px 14px', textAlign: 'center', fontSize: 11, fontWeight: 700, color: '#7c3aed', background: '#f5f3ff', textTransform: 'uppercase' as const }}>APTOGON</th>
                </tr>
              </thead>
              <tbody>
                {/* Marks and prices are data; only the row labels are prose. */}
                {([
                  { v: ['✗','✗','✗'] },
                  { v: ['✗','~','✗'] },
                  { v: ['✗','~','✗'] },
                  { v: ['$0.01–0.03','free*','$1–10'], text: true },
                ] as { v: string[]; text?: boolean }[])
                  .map((r, i) => ({ ...r, label: (t.raw('why.rows') as string[])[i] }))
                  .map((row, i) => (
                  <tr key={row.label} style={{ borderBottom: '1px solid #f8fafc', background: i % 2 ? '#fafafa' : '#fff' }}>
                    <td style={{ padding: '10px 14px', fontWeight: 600, color: '#374151', fontSize: 13 }}>{row.label}</td>
                    {row.v.map((val, j) => (
                      <td key={j} style={{ padding: '10px 14px', textAlign: 'center', fontWeight: 800, color: row.text ? '#6b7280' : val === '✗' ? '#dc2626' : val === '~' ? '#d97706' : '#16a34a' }}>{val}</td>
                    ))}
                    <td style={{ padding: '10px 14px', textAlign: 'center', fontWeight: 800, color: row.text ? '#7c3aed' : '#16a34a', background: '#f5f3ff' }}>
                      {row.text ? '~$0.01' : '✓'}
                    </td>
                  </tr>
                ))}
          ...
          <p style={{ textAlign: 'end', fontSize: '0.72rem', color: '#9ca3af', marginTop: 6 }}>
            {t('why.footnote')}
          </p>
        </div>
      </section>
