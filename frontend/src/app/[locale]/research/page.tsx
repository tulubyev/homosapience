        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
          <thead>
            <tr style={{ background: '#f8fafc', borderBottom: '1px solid #f1f5f9' }}>
              <th style={{ padding: '10px 12px', textAlign: 'left', fontSize: 11, fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase' }}>Provider</th>
              {CAPABILITIES.map(c => (
                <th key={c.id} style={{
                  padding: '10px 12px', textAlign: 'center', fontSize: 11, fontWeight: 700,
                  color: c.id === 'sybil_resistance' ? '#7c3aed' : '#94a3b8',
                  background: c.id === 'sybil_resistance' ? '#f5f3ff' : undefined,
                }}>
                  {c.label}
                </th>
              ))}
          ...
          <tbody>
            {COMPETITORS.map((row, i) => (
              <tr key={row.name} style={{ borderBottom: '1px solid #f8fafc', background: row.name === 'APTOGON' ? '#faf5ff' : i % 2 ? '#fafafa' : '#fff' }}>
                <td style={{ padding: '10px 12px', fontWeight: row.name === 'APTOGON' ? 800 : 600, color: '#1e1e2e' }}>
                  {row.name}
                  {row.note && <div style={{ fontSize: 11, fontWeight: 400, color: '#9ca3af', marginTop: 2 }}>{row.note}</div>}
                </td>
                {CAPABILITIES.map(c => {
                  const cell = CELL[row.values[c.id]]
                  return (
                    <td key={c.id} style={{
                      padding: '10px 12px', textAlign: 'center', fontWeight: 800, color: cell.color,
                      background: c.id === 'sybil_resistance' ? '#f5f3ff' : undefined,
                    }}>
                      {cell.sym}
                    </td>
