        ...
        {incomingRequests.length > 0 && (
          <section style={{ marginBottom: 40 }}>
            <h2 style={{ fontSize: '1.3rem', fontWeight: 900, color: '#111827', marginBottom: 6 }}>
              {t('incoming_title')} <span style={{ background: '#7c3aed', color: '#fff', borderRadius: 99, padding: '2px 10px', fontSize: '0.8rem', marginLeft: 6 }}>{incomingRequests.length}</span>
            </h2>
        ...
        <section id="request" style={{ marginBottom: 52 }}>
            ...
            {candidates.map(c => {
              ...
              return (
                <button key={c.did_hash_short} onClick={() => toggle(c.did_hash_short)}
                  style={{ textAlign: 'left', padding: '16px 18px', borderRadius: 16, border: `2px solid ${isSel ? '#7c3aed' : '#e9d5ff'}`, background: isSel ? '#faf5ff' : '#fff', cursor: 'pointer', transition: 'all 0.15s', boxShadow: isSel ? '0 4px 16px rgba(124,58,237,0.12)' : 'none' }}>
          ...
          <div style={{ marginTop: 20 }}>
            <button
              ...
              style={{ display: 'flex', alignItems: 'center', gap: 8, background: showKnownDids ? '#faf5ff' : '#fff', border: `1.5px solid ${showKnownDids ? '#c4b5fd' : '#e9d5ff'}`, borderRadius: 14, padding: '12px 20px', cursor: 'pointer', width: '100%', textAlign: 'left', transition: 'all 0.18s' }}
            >
              ...
              <span style={{ fontSize: '0.75rem', color: '#94a3b8', marginLeft: 4 }}>{showKnownDids ? '▲' : '▼'}</span>
            </button>
