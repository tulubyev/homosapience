...
import { getAccountBadge } from '@/lib/pairApi'
...
const NAV_LINKS = [
  ...
  { href: '/solutions',  label: 'Solutions',   color: '#fb923c', activeBg: 'rgba(251,146,60,0.12)',  activeBorder: 'rgba(251,146,60,0.35)'  },
  { href: '/developers', label: 'Developers',  color: '#67e8f9', activeBg: 'rgba(103,232,249,0.12)', activeBorder: 'rgba(103,232,249,0.35)' },
  { href: '/pricing',    label: 'Pricing',     color: '#34d399', activeBg: 'rgba(52,211,153,0.12)',  activeBorder: 'rgba(52,211,153,0.35)'  },
  { href: '/blog',       label: 'Blog',        color: '#f9a8d4', activeBg: 'rgba(249,168,212,0.12)', activeBorder: 'rgba(249,168,212,0.35)' },
...
// ── My Devices icon (visible only to verified users when DEVICE_ACCOUNTS is on) ──
function MyDevicesIcon({ mobile = false }: { mobile?: boolean }) {
  ...
  useEffect(() => {
    ...
    if (!did) return
    setShow(true)
    // Try to enrich tooltip with live count. Best-effort; if the feature flag is
    // off the call returns null and we keep a generic tooltip.
    ;(async () => {
      try {
        await autoRefreshSession()
        const b = await getAccountBadge()
        if (b) { setCount(b.device_count); setTrust(b.max_trust_label) }
      } catch { /* non-critical */ }
    })()
  }, [])
    ...
    return (
      <Link
        ...
        style={{
          ...
          textDecoration: 'none', color: '#c4b5fd', display: 'flex', alignItems: 'center', gap: 8,
        }}
      >
        🔐 My Account{count != null && count > 1 ? ` (${count})` : ''}
      </Link>
    )
  }

  const tooltip = count != null
    ? `My devices · ${count} linked${trust ? ` · ${trust}` : ''}`
    : 'My devices'
  ...
  return (
    ...
    >
      ...
      <span className="hdr-account-text">My Account</span>
      {count != null && count > 1 && (
        <span style={{
...
export default function Header() {
  ...
  // Incoming-message badge on the chat icon. Polls the public unread endpoint
  // using the last-read state the chat page stores in localStorage, so the count
  // updates even while browsing other pages.
  useEffect(() => {
    if (typeof localStorage === 'undefined') return
    const me = (localStorage.getItem('hsi_did') || localStorage.getItem('aptogon_did') || '').slice(-8)
    const poll = async () => {
      try {
        let since: Record<string, number> = {}
        try { since = JSON.parse(localStorage.getItem('chat_last_read') || '{}') } catch {}
        let baseline = Number(localStorage.getItem('chat_unread_baseline') || 0)
        if (!baseline) { baseline = Math.floor(Date.now() / 1000); localStorage.setItem('chat_unread_baseline', String(baseline)) }
        const r = await fetch('/api/chat/unread', {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ since, default_since: baseline, me: me || undefined }),
        })
        if (r.ok) setChatUnread((await r.json()).total || 0)
      } catch { /* non-critical */ }
    }
    poll()
    ...
    <>
      ...
      <header dir="ltr" style={{
        ...
        <div className="hdr-right">
          <Link href="/chat" title={chatUnread > 0 ? `Chat · ${chatUnread} new` : 'Chat'}
            style={{ position: 'relative', width: 32, height: 32, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: 8, background: 'rgba(244,114,182,0.1)', border: '1px solid rgba(244,114,182,0.2)', color: '#f472b6', textDecoration: 'none' }}>
            <ChatIcon />
            {chatUnread > 0 && (
              <span style={{ position: 'absolute', top: -5, right: -5, minWidth: 16, height: 16, padding: '0 4px', borderRadius: 99, background: '#ef4444', color: '#fff', fontSize: 9, fontWeight: 800, display: 'flex', alignItems: 'center', justifyContent: 'center', border: '2px solid rgba(10,15,26,0.95)' }}>
