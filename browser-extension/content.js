...
(function () {
  ...
  // Tiny DOM builder — the tooltip is assembled from createElement/textContent
  // instead of innerHTML, so a tampered credential field can never be parsed as
  // markup on any of the 23 host sites (defense in depth), and the AMO linter's
  // "unsafe innerHTML" warning never fires. Attrs: style→cssText, class→className,
  // everything else→setAttribute. String children become text nodes.
  ...
  const SITE_CONFIGS = [
    ...
    // LinkedIn
    {
      ...
      selectors: [
        ...
        // Feed/comment names render the visible copy inside span[aria-hidden]
        // plus a visually-hidden duplicate; target the aria-hidden one so
        // textContent is the name once, not doubled ("NameName").
        'span.feed-shared-actor__name span[aria-hidden="true"]',
        'span.comments-post-meta__name span[aria-hidden="true"]',
        'span.feed-shared-actor__name',         // fallback (older markup)
        'span.comments-post-meta__name',        // fallback (older markup)
        'a.app-aware-link span[aria-hidden="true"]',   // inline name
      ],
      type: 'inline',
    },
  ...
  function showBadgeTooltip(anchor, credential) {
    ...
    // A "label: value" line inside the details box.
    const infoRow = (label, valueNode) => h('div', null,
      h('span', { style: 'color:#9ca3af;' }, label), ' ', valueNode);

    tooltip.append(
      h('div', { style: 'display:flex;align-items:center;gap:8px;margin-bottom:10px;' },
        h('div', { style: 'width:32px;height:32px;border-radius:50%;background:linear-gradient(135deg,#7c3aed,#06b6d4);display:flex;align-items:center;justify-content:center;font-size:16px;flex-shrink:0;' }, '✦'),
        h('div', null,
          h('div', { style: 'font-weight:800;font-size:13px;color:#111827;' }, 'Verified Human'),
          h('div', { style: 'font-size:10px;color:#7c3aed;font-weight:600;' }, 'APTOGON · homosapience.org'),
        ),
      ),
      h('div', { style: 'background:#f9fafb;border-radius:8px;padding:10px 12px;margin-bottom:10px;font-size:11px;line-height:1.8;' },
        infoRow('DID:', h('span', { style: 'font-family:monospace;color:#374151;' }, shortDid)),
        infoRow('Confidence:', h('span', { style: 'color:#059669;font-weight:700;' }, conf + '%')),
        infoRow('Verified:', h('span', { style: 'color:#374151;' }, issued)),
        infoRow('Valid for:', h('span', { style: 'color:#374151;' }, daysLeft + ' days')),
      ),
      h('div', { style: 'font-size:10px;color:#9ca3af;text-align:center;' }, 'Gesture biometrics · Ed25519 · Aptos on-chain'),
    );
  ...
  // Normalize a username for comparison: collapse internal whitespace (LinkedIn
  // names arrive with newlines / duplicated a11y copies), trim, drop leading
  // u/ or @, lowercase.
  function normHandle(s) {
    return (s || '').replace(/\s+/g, ' ').trim().replace(/^u\//i, '').replace(/^@/, '').toLowerCase();
  }

  // True when an element's text is the holder's own handle. Exact match, plus a
  // whole-token prefix so decorated display names still match: LinkedIn appends
  // connection degree ("· 3rd"), pronouns ("(He/Him)"), etc. after the name.
  // The boundary check keeps "tulubyev" from matching "tulubyevski".
  function handleMatches(text, my) {
    if (!my) return false;
    const t = normHandle(text);
    if (t === my) return true;
    if (!t.startsWith(my)) return false;
    const next = t.charAt(my.length);
  ...
  function injectBadges() {
    ...
    for (const selector of config.selectors) {
      ...
      elements.forEach(el => {
        // Match BEFORE marking. SPA sites hydrate the name after the node
        // appears; marking a non-match on the first pass would consume it
        // permanently (the :not([INJECTED_ATTR]) filter then skips it forever),
        // so the badge never lands once the name finally renders.
        if (!handleMatches(el.textContent, myHandle)) return;
