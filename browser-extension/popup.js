...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Tiny DOM builder — every popup view is built from createElement/textContent
  // instead of innerHTML, so no string can ever be parsed as markup (defense in
  // depth against a tampered credential) and the AMO linter's "unsafe innerHTML"
  // warning never fires. Attrs: style→cssText, class→className, on*→listener,
  // everything else→setAttribute. Children: nodes kept as-is, anything else
  // becomes a text node.
  ...
  // Some i18n strings embed a literal "<br>" as a soft line break. Since views
  // are now built with textContent (not innerHTML), expand those markers into
  // real <br> nodes instead of printing the tag verbatim. Returns a child array.
  const brLines = (s) =>
    String(s).split(/<br\s*\/?>/i).flatMap((part, i) => (i === 0 ? [part] : [h('br'), part]));

  // Set loading text
  body.replaceChildren(h('div', { class: 'loading' }, t.loading));
      ...
      if (hsiTab) {
        chrome.scripting.executeScript({
          ...
          func: () => ({
            ...
            did:  localStorage.getItem('hsi_did') || localStorage.getItem('aptogon_did'),
            key:  localStorage.getItem('aptogon_key'),  // needed by background for signed-session JWT polling
          }),
        }, (results) => {
          const data = results?.[0]?.result;
          if (data?.cred) {
            chrome.storage.local.set({ hsi_credential: data.cred, hsi_did: data.did, aptogon_key: data.key || null }, () => {
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      ...
      chrome.scripting.executeScript({
        ...
        func: () => ({
          cred: localStorage.getItem('hsi_credential'),
          did:  localStorage.getItem('hsi_did') || localStorage.getItem('aptogon_did'),
          key:  localStorage.getItem('aptogon_key'),
        }),
      }, (results) => {
        if (chrome.runtime.lastError || !results || !results[0]) {
          body.appendChild(h('p', { style: 'color:#ef4444;font-size:11px;text-align:center;margin-top:8px;' },
            t.errRead + ' ',
            h('a', { href: 'https://homosapience.org/verify', target: '_blank', style: 'color:#a78bfa' }, 'homosapience.org/verify')));
          return;
        }
        const { cred, did, key } = results[0].result;
        if (cred) {
          chrome.storage.local.set({ hsi_credential: cred, hsi_did: did, aptogon_key: key || null }, () => {
            window.location.reload();
          });
        } else {
          body.appendChild(h('p', { style: 'color:#f59e0b;font-size:11px;text-align:center;margin-top:8px;' },
            t.errNotFound + ' ',
            h('a', { href: 'https://homosapience.org/verify', target: '_blank', style: 'color:#a78bfa' }, 'homosapience.org/verify')));
        }
      });
  ...
  function renderBondRequestsBanner(requests) {
    ...
    `;
    const confColor = conf >= 80 ? '#4ade80' : conf >= 70 ? '#facc15' : '#f87171';
    banner.append(
      h('div', { style: 'display:flex;align-items:center;gap:8px;margin-bottom:10px;' },
        h('span', { style: 'font-size:18px;' }, '🤝'),
        h('div', null,
          h('div', { style: 'font-weight:800;font-size:12px;color:#fff;' },
            t.bondRequest,
            requests.length > 1
              ? h('span', { style: 'background:rgba(167,139,250,0.3);color:#c4b5fd;border-radius:99px;padding:1px 7px;font-size:10px;margin-left:4px;' }, '+' + (requests.length - 1))
              : null,
          ),
          h('div', { style: 'font-size:10px;color:#a78bfa;font-family:monospace;' }, shortDid),
        ),
        h('div', { style: `margin-left:auto;font-size:11px;font-weight:800;color:${confColor};` }, conf + '%'),
      ),
      h('div', { style: 'display:flex;gap:8px;' },
        h('button', { id: 'bond-approve', style: 'flex:1;padding:9px;border-radius:10px;border:none;cursor:pointer;background:linear-gradient(135deg,#059669,#10b981);color:#fff;font-weight:700;font-size:12px;' }, '✅ ' + t.vouch),
        h('button', { id: 'bond-reject', style: 'flex:1;padding:9px;border-radius:10px;cursor:pointer;background:rgba(255,255,255,0.1);color:#c4b5fd;font-weight:700;font-size:12px;border:1px solid rgba(167,139,250,0.3);' }, '❌ ' + t.decline),
      ),
    );
    ...
    document.getElementById('bond-approve').addEventListener('click', async () => {
      ...
      try {
        ...
        banner.replaceChildren(h('div', { style: 'text-align:center;padding:10px;color:#4ade80;font-weight:700;font-size:12px;' }, '✅ ' + t.bondApproved));
  ...
  function renderValid(cred) {
    ...
    const txNode = txIsReal
      ? h('a', { href: `https://explorer.aptoslabs.com/txn/${encodeURIComponent(txRaw)}?network=testnet`, target: '_blank', style: 'color:#7c3aed;font-family:monospace;font-size:11px;' }, txRaw.slice(0, 10) + '...')
      : h('span', { style: 'color:#94a3b8;font-size:11px;' }, t.testnetSoon);

    // Shorthand for a label/value details row.
    const row = (label, valueNode) => h('div', { class: 'detail-row' },
      h('span', { class: 'detail-label' }, label),
      valueNode);
    const myDevicesLabel = t.myDevices || '🔐 My devices';

    body.replaceChildren(
      h('div', { class: 'status-card valid' },
        h('div', { class: 'status-icon' }, '✅'),
        h('div', { class: 'status-title valid' }, t.verified),
        h('div', { class: 'status-sub' }, ...brLines(t.verifiedSub)),
      ),
      h('div', { class: 'details' },
        h('div', { class: 'detail-row', style: 'flex-direction:column;align-items:flex-start;gap:6px;padding-bottom:12px;' },
          h('span', { class: 'detail-label' }, 'DID'),
          h('div', { style: 'display:flex;align-items:center;justify-content:space-between;width:100%;gap:6px;' },
            h('span', { class: 'detail-value', style: 'font-size:10px;word-break:break-all;flex:1;', title: did }, shortDid),
            h('button', { id: 'btn-copy-did', style: 'flex-shrink:0;padding:3px 10px;border-radius:7px;border:1px solid rgba(124,58,237,0.3);background:rgba(124,58,237,0.08);color:#7c3aed;font-weight:700;font-size:10px;cursor:pointer;white-space:nowrap;transition:all 0.2s;' }, t.copy),
          ),
        ),
        row(t.aiConf, h('span', { class: 'detail-value good' }, conf + '%')),
        row(t.issuedOn, h('span', { class: 'detail-value' }, issued)),
        row(t.validFor, h('span', { class: 'detail-value ' + (daysLeft <= 7 ? 'warn' : 'good') }, daysLeft + ' ' + t.days)),
        row('Aptos TX', h('span', { class: 'detail-value' }, txNode)),
        row(t.onchain, h('span', { class: 'detail-value', id: 'onchain-status' }, t.checking)),
        row(t.bonds, h('span', { class: 'detail-value', id: 'bond-count' }, '…')),
      ),
      h('button', { class: 'btn btn-secondary', id: 'btn-qr' }, t.didAsQr),
      h('button', { class: 'btn btn-secondary', id: 'btn-my-devices', title: myDevicesLabel }, myDevicesLabel),
      h('button', { class: 'btn btn-secondary', id: 'btn-refresh' }, t.refresh),
    );
    ...
    // My Devices — open /account page (DEVICE_ACCOUNTS feature) in a new tab.
    // The /account page itself fetches the full list (with a proper Bearer
    // session it acquires there). We don't try to fetch a count from the
    // popup because /api/pair/account now requires a signed Bearer JWT
    // (Variant A hardening) — the popup has no session and would just 401.
    // The page handles the flag-off case with a friendly disabled message.
    document.getElementById('btn-my-devices').addEventListener('click', () => {
      const ui = (navigator.language || 'en').slice(0, 2);
      const lang = ['en','ru','de','es','fr','pt','ar','he','hi','ja','zh'].includes(ui) ? ui : 'en';
      chrome.tabs.create({ url: `https://homosapience.org/${lang}/account` });
    });
  ...
  function renderExpired() {
    body.replaceChildren(
      h('div', { class: 'status-card expired' },
        h('div', { class: 'status-icon' }, '⏰'),
        h('div', { class: 'status-title expired' }, t.expired),
        h('div', { class: 'status-sub' }, ...brLines(t.expiredSub)),
      ),
      h('button', { class: 'btn btn-primary', id: 'btn-verify' }, t.btnVerify),
      h('button', { class: 'btn btn-secondary', id: 'btn-sync' }, t.btnSync),
    );
  ...
  function renderNone() {
    body.replaceChildren(
      h('div', { class: 'status-card none' },
        h('div', { class: 'status-icon' }, '👤'),
        h('div', { class: 'status-title none' }, t.notVerified),
        h('div', { class: 'status-sub' }, ...brLines(t.notVerifiedSub)),
      ),
      h('div', { style: 'background:#fff;border-radius:12px;border:1px solid #e9d5ff;padding:12px 14px;margin-bottom:14px;font-size:11px;color:#6b7280;line-height:1.7;' },
        h('strong', { style: 'color:#7c3aed;' }, t.benefits), h('br'),
        t.b1, h('br'),
        t.b2, h('br'),
        t.b3, h('br'),
        t.b4,
